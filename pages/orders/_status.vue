  ...
  <v-card class="ma-1" elevation="0">
    ...
    <v-component-fullscreen>
      ...
      <v-tabs color="primary">
        <v-tab exact to="?assigning=3">{{ $vuetify.lang.t('$vuetify.lang_64') }}</v-tab>
        <v-tab exact to="?assigning=1">{{ $vuetify.lang.t('$vuetify.lang_140') }}</v-tab>
        <v-tab exact to="?assigning=0">{{ $vuetify.lang.t('$vuetify.lang_141') }}</v-tab>
      </v-tabs>
      <!-- End: tabs bar -->
...
<script>
  ...
  export default {
    ...
    watch: {
       ...
       */
      $route(params) {
        this.getOrders(params.query.assigning);
      }
    },
    mounted() {
      this.getOrders();
    ...
    methods: {
      ...
      getOrders(assigning) {
        this.$axios.$post(Api.exchange.getOrders, {
          owner: true,
          assigning: assigning ? Number(assigning) : 3,
          status: this.status,
          limit: 15,
          page: this.page
        }).then((response) => {
       ...
       */
      getMore() {
        this.getOrders(this.$route.query.assigning);
      },
