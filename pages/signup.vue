    ...
    <v-flex mx5 sm6 md4 xl3 align-self-center>
      ...
      <template v-else>
        ...
        <v-stepper class="transparent" v-model="next" vertical flat>
          ...
          <!-- Start: step auth signup: 1 -->
          <v-stepper-step color="black--text yellow darken-1" :complete="next > 1" step="1">
            {{ $vuetify.lang.t('$vuetify.lang_1') }}
            <small>{{ $vuetify.lang.t('$vuetify.lang_2') }}</small>
          </v-stepper-step>
          <v-stepper-content step="1">
            <v-form class="mt-2" ref="form">
              <v-text-field color="yellow darken-3" outlined :label="$vuetify.lang.t('$vuetify.lang_3')" v-model="name" :rules="rulesName" :counter="5" required>
              ...
              <v-text-field color="yellow darken-3" outlined :label="$vuetify.lang.t('$vuetify.lang_5')" v-model="password" :rules="rulesPassword" :counter="8" :append-icon="print ? 'mdi-eye' : 'mdi-eye-off'" :type="print ? 'text' : 'password'" @click:append="print = !print" required>
                <template #message="{ message }">
                  {{ $vuetify.lang.t(message) }}
                </template>
              </v-text-field>
              <v-text-field color="yellow darken-3" outlined :label="$vuetify.lang.t('$vuetify.lang_9')" v-model="confirm" :rules="rulesConfirm" :type="print ? 'text' : 'password'" required>
                <template #message="{ message }">
                  {{ $vuetify.lang.t(message) }}
                </template>
              </v-text-field>
              <v-btn color="black--text yellow darken-1 text-capitalize" large block elevation="0" @click="actionSignup(0)">{{ $vuetify.lang.t('$vuetify.lang_40') }}</v-btn>
              <div class="mt-3">{{ $vuetify.lang.t('$vuetify.lang_7') }} <nuxt-link class="orange--text text-decoration-none" to="/signin">{{ $vuetify.lang.t('$vuetify.lang_29') }}</nuxt-link></div>
            </v-form>
          </v-stepper-content>
          <!-- End: step auth signup: 1 -->

          <!-- Start: step auth signup: 2 -->
          <v-stepper-step color="black--text yellow darken-1" :complete="next > 2" step="2">
            {{ $vuetify.lang.t('$vuetify.lang_19') }}
            <small>{{ $vuetify.lang.t('$vuetify.lang_21') }}</small>
          </v-stepper-step>
          <v-stepper-content step="2">
            <v-text-field class="mt-2" color="yellow darken-3" outlined :label="$vuetify.lang.t('$vuetify.lang_35')" v-model="secure" :counter="6" :hint="$vuetify.lang.t('$vuetify.lang_43') + email">
